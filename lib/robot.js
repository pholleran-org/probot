 ...
 */
class Robot {
  constructor({cache, logger, router, catchErrors, app} = {}) {
  ...
  on(event, callback) {
    ...
    return this.events.on(name, async event => {
      if (!action || action === event.payload.action) {
        try {
          const github = await this.auth('installation' in event.payload ? event.payload.installation.id : null);
  ...
  async auth(id) {
    ...
    if (process.env.AUTH_METHOD === 'githubapp') {
      ...
      github = new GitHubApi({debug: process.env.LOG_LEVEL === 'trace'});
      const res = await this.cache.wrap(`app:oauth${process.env.CLIENT_ID}:token`, () => {
      ...
      */
      github = new GitHubApi({debug: process.env.LOG_LEVEL === 'trace'});
      github.authenticate({type: 'oauth', token: process.env.TOKEN});
