 ...
 */
class Robot {
  constructor({cache, logger, catchErrors, app} = {}) {
    this.app = app;
...
//          Const github = await this.auth(event.payload.installation.id);
  ...
  async auth(id) {
    ...
    if (process.env.AUTH_METHOD === 'githubapp') {
      ...
      github = new GitHubApi({debug: process.env.LOG_LEVEL === 'trace', host: 'octodemo.com', pathPrefix: '/api/v3'});
      github.authenticate({type: 'token', token: process.env.OCTODEMO_HUBOT});
